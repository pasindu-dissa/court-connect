...
class _MatchmakingScreenState extends State<MatchmakingScreen> {
  ...
  Future<void> _updateMarkers() async {
    ...
    final BitmapDescriptor matchIcon = await MarkerGenerator.createCustomMarkerBitmap("Match", color: AppColors.primary);
  ...
  List<Map<String, dynamic>> get _filteredMatches {
    return _matches.where((match) {
      final matchesSearch = (match['title'] ?? match['courtName'] ?? "").toString().toLowerCase().contains(_searchQuery.toLowerCase());
      final matchesSport = _sportFilter == null || match['sport'] == _sportFilter;
      final matchesSkill = _skillFilter == null || match['skill'] == _skillFilter;
      final matchesLocation = _locationFilter == null || (match['location'] ?? "").toString().toLowerCase().contains(_locationFilter!.toLowerCase());
      ...
      if (_timeFilter != null) {
        ...
        if (timeStr.isNotEmpty) {
          ...
          if (_timeFilter == "Morning") matchesTime = (hour >= 5 && hour < 12);
          else if (_timeFilter == "Afternoon") matchesTime = (hour >= 12 && hour < 17);
          else if (_timeFilter == "Evening") matchesTime = (hour >= 17 && hour < 20);
          else if (_timeFilter == "Night") matchesTime = (hour >= 20 || hour < 5);
        }
      }

      return matchesSearch && matchesSport && matchesSkill && matchesLocation && matchesTime;
    }).toList();
  }
    ...
    return Scaffold(
      ...
      floatingActionButton: !_isMapView ? Padding(
        ...
        child: FloatingActionButton.extended(
          onPressed: () async {
            final newMatch = await Navigator.push(context, MaterialPageRoute(builder: (_) => const CreateMatchScreen()));
            if (newMatch != null) _loadData(); 
          },
          backgroundColor: AppColors.primary,
          icon: const Icon(Icons.add_circle_outline, color: Colors.white),
          label: const Text("Host Match", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ),
      ) : null,
                    ...
                    Row(
                      children: [
                        ...
                        _buildGlassIconBtn(
                          context, 
                          icon: Icons.tune, 
                          isActive: _sportFilter != null || _skillFilter != null || _timeFilter != null || _locationFilter != null,
                ...
                AnimatedContainer(
                  ...
                  child: _isSearchVisible ? TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: "Search matches...", filled: true, fillColor: Theme.of(context).cardColor,
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  radius: 30,
                  backgroundImage: NetworkImage(match['image'] ?? "https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5"),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(match['title'] ?? match['courtName'] ?? 'Match', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                      const SizedBox(height: 4),
                      Text("${match['sport']} • ${match['time']}", style: TextStyle(color: Colors.grey[600], fontSize: 14)),
                      const SizedBox(height: 8),
                      Text("LKR ${match['fee'] ?? 0} per person", style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ],
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: isHost ? Colors.orange : AppColors.primary,
  ...
  Widget _buildGlassButton(BuildContext context, {required IconData icon, required String label, required VoidCallback onTap, bool isActive = false}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: isActive ? AppColors.primary : Theme.of(context).cardColor.withValues(alpha: 0.9), borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 10)]),
        child: Row(children: [
  ...
  Widget _buildGlassIconBtn(BuildContext context, {required IconData icon, required VoidCallback onTap, bool isActive = false}) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.all(10),
        decoration: BoxDecoration(color: isActive ? AppColors.primary : Theme.of(context).cardColor.withValues(alpha: 0.9), shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 10)]),
        child: Icon(icon, size: 20, color: isActive ? Colors.white : Theme.of(context).iconTheme.color),
      ),
    );
  }
}